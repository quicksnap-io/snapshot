...
<template>
  <Listbox v-model="selectedItem" as="div" :disabled="disableInput">
    ...
    <div class="relative">
      <ListboxButton
        :class="[
          'relative h-[42px] w-full truncate rounded-full border border-skin-border pl-3 pr-[40px] text-left ',
          {
            'cursor-not-allowed text-skin-border': disableInput,
            'text-skin-link hover:border-skin-text': !disableInput
          }
        ]"
      >
        <slot
      ...
      >
        <ListboxOptions
          class="absolute z-40 mb-4 mt-1 w-full overflow-hidden rounded-md border border-skin-border bg-skin-bg text-base shadow-lg focus:outline-none sm:text-sm"
        >
